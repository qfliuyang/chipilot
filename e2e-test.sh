#!/bin/bash
# E2E Test with Real Visual Evidence
# Usage: ./e2e-test.sh

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TIMESTAMP=$(date +%Y%m%d-%H%M%S)
OUTPUT_DIR="$SCRIPT_DIR/tests/output/e2e-$TIMESTAMP"
SESSION_NAME="chipilot-e2e-$TIMESTAMP"

mkdir -p "$OUTPUT_DIR"

# Colors
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m'

log_info() { echo -e "${BLUE}[E2E]${NC} $1" | tee -a "$OUTPUT_DIR/e2e.log"; }
log_success() { echo -e "${GREEN}[PASS]${NC} $1" | tee -a "$OUTPUT_DIR/e2e.log"; }
log_warn() { echo -e "${YELLOW}[WARN]${NC} $1" | tee -a "$OUTPUT_DIR/e2e.log"; }
log_error() { echo -e "${RED}[FAIL]${NC} $1" | tee -a "$OUTPUT_DIR/e2e.log"; }

cleanup() {
    log_info "Cleaning up..."
    tmux kill-session -t "$SESSION_NAME" 2>/dev/null || true
    pkill -f "node dist/cli.js" 2>/dev/null || true
    # Stop any running ffmpeg
    if [ -f "$OUTPUT_DIR/recording.pid" ]; then
        kill $(cat "$OUTPUT_DIR/recording.pid") 2>/dev/null || true
    fi
}

trap cleanup EXIT

# Check for recording tools
check_tools() {
    log_info "Checking recording tools..."

    # macOS screencapture
    if [[ "$OSTYPE" == "darwin"* ]] && command -v screencapture &> /dev/null; then
        SCREENSHOT_TOOL="screencapture"
        log_success "macOS screencapture available"
    else
        SCREENSHOT_TOOL="none"
        log_warn "No native screenshot tool - using tmux capture only"
    fi

    # Check ffmpeg for video
    if command -v ffmpeg &> /dev/null; then
        FFMPEG_AVAILABLE=true
        log_success "ffmpeg available for video recording"
    else
        FFMPEG_AVAILABLE=false
        log_warn "ffmpeg not available - video recording disabled"
    fi

    # Check tmux
    if ! command -v tmux &> /dev/null; then
        log_error "tmux is required but not installed"
        exit 1
    fi
    log_success "tmux available"
}

# Take screenshot using screencapture (actual PNG)
take_screenshot() {
    local name="$1"
    local output="$OUTPUT_DIR/${name}.png"

    if [ "$SCREENSHOT_TOOL" = "none" ]; then
        log_warn "Screenshot tool not available"
        return 1
    fi

    log_info "Taking screenshot: $name.png"

    # Capture the active terminal window
    screencapture -l$(osascript -e 'tell app "Terminal" to id of window 1' 2>/dev/null || echo "0") "$output" 2>/dev/null || \
    screencapture "$output" 2>/dev/null || true

    if [ -f "$output" ] && [ -s "$output" ]; then
        local size=$(stat -f%z "$output" 2>/dev/null || stat -c%s "$output" 2>/dev/null || echo "unknown")
        log_success "Screenshot saved: $name.png ($size bytes)"
        return 0
    else
        log_warn "Screenshot may have failed"
        return 1
    fi
}

# Start video recording
start_recording() {
    local duration="${1:-60}"
    local output="$OUTPUT_DIR/recording.mp4"

    if [ "$FFMPEG_AVAILABLE" = false ]; then
        log_warn "Video recording disabled"
        return 1
    fi

    log_info "Starting screen recording (${duration}s)..."

    if [[ "$OSTYPE" == "darwin"* ]]; then
        # macOS screen recording - capture main display
        ffmpeg -f avfoundation -i "1:none" -t "$duration" -pix_fmt yuv420p -movflags +faststart "$output" 2>/dev/null &
    else
        # Linux screen recording
        ffmpeg -f x11grab -i :0.0 -t "$duration" -pix_fmt yuv420p -movflags +faststart "$output" 2>/dev/null &
    fi

    VIDEO_PID=$!
    echo "$VIDEO_PID" > "$OUTPUT_DIR/recording.pid"
    log_success "Recording started (PID: $VIDEO_PID)"
}

# Stop recording
stop_recording() {
    if [ -f "$OUTPUT_DIR/recording.pid" ]; then
        local pid=$(cat "$OUTPUT_DIR/recording.pid")
        log_info "Stopping recording..."
        kill $pid 2>/dev/null || true
        rm -f "$OUTPUT_DIR/recording.pid"
        sleep 2

        if [ -f "$OUTPUT_DIR/recording.mp4" ]; then
            local size=$(stat -f%z "$OUTPUT_DIR/recording.mp4" 2>/dev/null || stat -c%s "$OUTPUT_DIR/recording.mp4" 2>/dev/null || echo "unknown")
            log_success "Video saved: recording.mp4 ($size bytes)"
        fi
    fi
}

# Launch chipilot in tmux
launch_chipilot() {
    log_info "Launching chipilot in tmux session: $SESSION_NAME"

    cd "$SCRIPT_DIR"

    # Build if needed
    if [ ! -f "dist/cli.js" ]; then
        log_info "Building project..."
        npm run build
    fi

    # Kill existing session
    tmux kill-session -t "$SESSION_NAME" 2>/dev/null || true

    # Create new session
    tmux new-session -d -s "$SESSION_NAME" -n "chipilot"
    tmux send-keys -t "$SESSION_NAME" "cd '$SCRIPT_DIR'" C-m
    tmux send-keys -t "$SESSION_NAME" "clear" C-m
    tmux send-keys -t "$SESSION_NAME" "node dist/cli.js" C-m

    log_success "Chipilot launched in tmux"
    log_info "Waiting 5 seconds for startup..."
    sleep 5
}

# Capture tmux pane (text capture for verification)
capture_pane() {
    local name="$1"
    tmux capture-pane -t "$SESSION_NAME" -p > "$OUTPUT_DIR/${name}.txt"
    log_info "Captured pane: $name.txt"
}

# Send keys to tmux
send_keys() {
    local keys="$1"
    local delay="${2:-0.5}"
    tmux send-keys -t "$SESSION_NAME" "$keys"
    sleep "$delay"
}

# Run E2E test sequence
run_e2e_test() {
    log_info "Starting E2E Test Sequence"

    # Screenshot 1: Initial state
    take_screenshot "01-initial"
    capture_pane "01-initial"
    sleep 2

    # Tab to terminal
    log_info "Switching to terminal pane..."
    send_keys "Tab" 1
    take_screenshot "02-terminal-focused"
    capture_pane "02-terminal-focused"

    # Type command
    log_info "Typing command..."
    send_keys "echo E2E_TEST_SUCCESS" 0.5
    take_screenshot "03-command-typed"
    capture_pane "03-command-typed"

    # Execute
    log_info "Executing command..."
    send_keys "Enter" 2
    take_screenshot "04-command-executed"
    capture_pane "04-command-executed"

    # Switch back to chat
    log_info "Switching to chat pane..."
    send_keys "Tab" 1
    take_screenshot "05-chat-focused"
    capture_pane "05-chat-focused"

    # Type chat message
    log_info "Typing chat message..."
    send_keys "Hello from E2E test" 0.5
    take_screenshot "06-chat-typed"
    capture_pane "06-chat-typed"

    # Submit
    log_info "Submitting message..."
    send_keys "Enter" 3
    take_screenshot "07-chat-response"
    capture_pane "07-chat-response"

    # Final screenshot
    sleep 2
    take_screenshot "08-final"
    capture_pane "08-final"

    log_success "E2E test sequence complete"
}

# Verify test results
verify_results() {
    log_info "Verifying test results..."

    local passed=0
    local failed=0

    # Check for command output
    if grep -q "E2E_TEST_SUCCESS" "$OUTPUT_DIR/04-command-executed.txt" 2>/dev/null; then
        log_success "Terminal command executed successfully"
        passed=$((passed + 1))
    else
        log_warn "Command output not detected"
        failed=$((failed + 1))
    fi

    # Check for chat response
    if grep -q "Hello from E2E test\|You said:" "$OUTPUT_DIR/07-chat-response.txt" 2>/dev/null; then
        log_success "Chat message processed"
        passed=$((passed + 1))
    else
        log_warn "Chat response not detected"
        failed=$((failed + 1))
    fi

    # Check screenshots
    local screenshot_count=$(ls -1 "$OUTPUT_DIR"/*.png 2>/dev/null | wc -l)
    if [ "$screenshot_count" -gt 0 ]; then
        log_success "$screenshot_count PNG screenshot(s) captured"
        passed=$((passed + 1))
    else
        log_warn "No PNG screenshots captured"
        failed=$((failed + 1))
    fi

    # Check video
    if [ -f "$OUTPUT_DIR/recording.mp4" ]; then
        log_success "Video recording captured"
        passed=$((passed + 1))
    else
        log_warn "No video recording"
    fi

    log_info "Verification: $passed passed, $failed failed"
    return $failed
}

# Generate report
generate_report() {
    log_info "Generating E2E report..."

    cat > "$OUTPUT_DIR/E2E-REPORT.md" << EOF
# E2E Test Report with Visual Evidence

**Date:** $(date)
**Test ID:** e2e-$TIMESTAMP
**Output Directory:** $OUTPUT_DIR

## Files Generated

### Screenshots (PNG)
$(ls -lh $OUTPUT_DIR/*.png 2>/dev/null | awk '{print "- " $9 " (" $5 ")"}' || echo "- No PNG screenshots")

### Text Captures
$(ls -lh $OUTPUT_DIR/*.txt 2>/dev/null | awk '{print "- " $9 " (" $5 ")"}' || echo "- No text captures")

### Video
$(ls -lh $OUTPUT_DIR/recording.mp4 2>/dev/null | awk '{print "- " $9 " (" $5 ")"}' || echo "- No video recording")

### Logs
- e2e.log

## Test Steps Executed

1. **Initial Load** - App startup screenshot
2. **Terminal Focus** - Switched to terminal pane
3. **Command Entry** - Typed test command
4. **Command Execution** - Executed command
5. **Chat Focus** - Switched to chat pane
6. **Chat Entry** - Typed chat message
7. **Chat Submit** - Submitted message
8. **Final State** - End of test

## System Info

- **OS:** $(uname -s)
- **Screenshot Tool:** $SCREENSHOT_TOOL
- **Video Recording:** $([ "$FFMPEG_AVAILABLE" = true ] && echo "Yes" || echo "No")

## Results

$(cat "$OUTPUT_DIR/e2e.log" | grep -E "\[PASS\]|\[FAIL\]|\[WARN\]")

## Visual Evidence

View screenshots:
\`\`\`bash
open $OUTPUT_DIR/*.png
\`\`\`

Play video:
\`\`\`bash
open $OUTPUT_DIR/recording.mp4
\`\`\`

EOF

    log_success "Report: $OUTPUT_DIR/E2E-REPORT.md"
}

# Main
main() {
    echo "========================================"
    echo "E2E TEST WITH VISUAL EVIDENCE"
    echo "========================================"
    echo ""
    log_info "Output: $OUTPUT_DIR"
    log_info "Session: $SESSION_NAME"

    # Setup
    check_tools
    launch_chipilot

    # Start recording
    start_recording 45
    sleep 3

    # Run tests
    run_e2e_test

    # Stop recording
    sleep 2
    stop_recording

    # Verify and generate report
    verify_results
    generate_report

    echo ""
    echo "========================================"
    echo "E2E TEST COMPLETE"
    echo "========================================"
    echo ""
    log_success "Results in: $OUTPUT_DIR"

    # Summary
    echo ""
    echo "Summary:"
    ls -lh "$OUTPUT_DIR"

    # Show PNG files specifically
    echo ""
    echo "PNG Screenshots:"
    ls -lh "$OUTPUT_DIR"/*.png 2>/dev/null || echo "  (none)"

    # Show video
    echo ""
    echo "Video Recording:"
    ls -lh "$OUTPUT_DIR"/*.mp4 2>/dev/null || echo "  (none)"
}

main "$@"
