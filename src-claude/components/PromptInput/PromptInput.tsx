...
function PromptInput({
  ...
  // Handler for chat:terminalModeToggle - toggle terminal execution mode
  const handleTerminalModeToggle = useCallback(() => {
    setTerminalModeEnabled(prev => {
      const newValue = !prev;
      // Show notification when toggled
      addNotification({
        key: 'terminal-mode-toggled',
        content: <Text color="bashBorder">
            {newValue ? 'Terminal mode enabled - bash commands will run in terminal pane' : 'Terminal mode disabled - bash commands will run inline'}
          </Text>,
        priority: 'immediate',
        timeoutMs: 3000
      });
  ...
  const chatHandlers = useMemo(() => ({
    ...
    'chat:terminalModeToggle': handleTerminalModeToggle,
    'chat:cycleMode': handleCycleMode,
    'chat:imagePaste': handleImagePaste
  }), [handleUndo, handleNewline, handleExternalEditor, handleStash, handleModelPicker, handleThinkingToggle, handleTerminalModeToggle, handleCycleMode, handleImagePaste]);
  ...
  return <Box flexDirection="column" marginTop={briefOwnsGap ? 0 : 1}>
      ...
      {swarmBanner ? <>
        ...
        </> : <Box flexDirection="row" alignItems="flex-start" justifyContent="flex-start" borderColor={getBorderColor()} borderStyle="round" borderLeft={false} borderRight={false} borderBottom width="100%" borderText={buildBorderText(showFastIcon ?? false, showFastIconHint, fastModeCooldown, terminalModeEnabled)}>
          <PromptInputModeIndicator mode={mode} isLoading={isLoading} viewingAgentName={viewingAgentName} viewingAgentColor={viewingAgentColor} />
          <Box flexGrow={1} flexShrink={1} onClick={handleInputClick}>
            {textInputElement}
          </Box>
...
function buildBorderText(showFastIcon: boolean, showFastIconHint: boolean, fastModeCooldown: boolean, terminalModeEnabled: boolean): BorderTextOptions | undefined {
  const segments: string[] = [];

  if (terminalModeEnabled) {
    segments.push(chalk.hex('#FF6B35')('[TERM]'));
  }
